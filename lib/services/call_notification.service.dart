...
class CallNotificationService {
  ...
  Future<void> _checkForIncomingCalls() async {
    ...
    try {
      ...
      final directory = Directory('/storage/emulated/0/Download');
      if (await directory.exists()) {
        // Check for notifications from customer app (the other app)
        final customerFile = File(
          '${directory.path}/geomart_call_notification_${_currentOrderId}_customer.json',
        );

        print(
          'CallNotification: Checking for customer file: ${customerFile.path}',
        );
        ...
        if (await customerFile.exists()) {
          ...
          print(
            'CallNotification: Found customer notification file with data: $callData',
          );

          // This is from the customer app, so process it
          final callerApp = callData['callerApp'] ?? '';
          if (callerApp == 'customer') {
            print(
              'CallNotification: Processing incoming call from customer app',
            );
            ...
            _showIncomingCallScreen(
              ...
              callerType: callData['callerType'] ?? 'customer',
        ...
        if (await driverFile.exists()) {
          ...
          if (callerApp == 'driver') {
            print(
              'CallNotification: Ignoring own notification from driver app',
            );
            return;
          }
        }
        ...
        MaterialPageRoute(
          builder:
              (context) => IncomingCallScreen(
                callId: 'call_${DateTime.now().millisecondsSinceEpoch}', // Generate a call ID
                callerName: callerName,
                callerPhoto: '', // Default empty photo
    ...
    try {
      ...
      if (await directory.exists()) {
        ...
        final callData = {
          ...
          'callerApp': 'driver', // Identify which app sent the notification
        };
