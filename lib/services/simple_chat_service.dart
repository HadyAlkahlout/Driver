...
class ChatMessage {
  ...
  factory ChatMessage.fromJson(Map<String, dynamic> json) {
    return ChatMessage(
      ...
      senderType: json['senderType'] ?? json['sender_type'] ?? 'driver',
...
class SimpleChatService {
  ...
  void startListening(String orderId) {
    print('Driver: Starting to listen for order: $orderId');
      ...
      if (response.statusCode == 200) {
        ...
        if (!_areMessagesEqual(_messages, newMessages) ||
          ...
          if (newMessages.length > _lastMessageCount && _lastMessageCount > 0) {
            ...
            print('Driver: New message detected! Count: ${newMessages.length}');
          }
          ...
          print('Driver: Updated messages - count: ${newMessages.length}');
        }
      }
    } catch (e) {
  ...
  Future<bool> sendMessage({
    ...
    try {
      ...
      final token = await AuthServices.getAuthBearerToken();
      print('Driver: Sending message to: $baseUrl/orders/$orderId/messages');
      ...
      print('Driver: Request body: $requestBody');
      ...
      print('Driver: Message response status: ${response.statusCode}');
      print('Driver: Message response body: ${response.body}');
